...
function Renderer(container) {
    ...
    this.init = function(_image, _imageType, _dynamic, haov, vaov, voffset, callback, params) {
        ...
        // Этот ужасный тест, специфичный для браузера, существует, потому что iOS 8/9 и IE 11
        // не отображать текстуры кубической карты, не являющиеся степенью двойки, но и не отображать их.
        //выдаем ошибку (проверено на iPhone 5c/iOS 8.1.3/iOS 9.2/
        // iOS 10.3.1).
        // Следовательно, контекст WebGL никогда не создается для этих браузеров для
        // Кубические карты NPOT и резервный рендерер CSS 3D-преобразования.
        // вместо.
        if (!(imageType == 'cubemap' &&
        ...
        // Если WebGL отсутствует, вернитесь к средству рендеринга 3D-преобразования CSS.
        // Это приведет к отмене загруженного изображения и загрузке резервного изображения.
        // Хотя тесты, специфичные для браузера, обычно не одобряются,
        // Резервный просмотрщик реально работает только с WebKit/Blink и IE 10/11
        // (в Firefox это работает некорректно).
